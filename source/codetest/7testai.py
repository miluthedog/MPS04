...
try:
    ...
    # Pass PSD through a mini subnetwork manually
    psd_processed = tf.keras.Sequential([
        tf.keras.layers.InputLayer(input_shape=(psd_features.shape[1],)),
        tf.keras.layers.Dense(32, activation='relu'),
        tf.keras.layers.Dense(16, activation='relu'),
    ])(psd_features)

    # Then stack processed PSD with sk_features
    X = np.hstack((psd_processed, sk_features))
    ...
    # Configure callbacks to minimize printing
    ...
    # Save model
    model.save("psd_model.h5")
    print("Model saved to psd_model.h5")
